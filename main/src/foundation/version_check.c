...
static esp_err_t version_check_http_event_handler(esp_http_client_event_t* evt) {
  ...
  switch (evt->event_id) {
    ...
    case HTTP_EVENT_ON_FINISH:
      ...
      check_and_free((void**)&output_buffer);
      output_len = 0;
      break;
...
static esp_err_t get_https_version(char const* const url_version, char* version) {
  ...
  check_and_free((void**)&local_response_buffer);
  return err;
}
...
static esp_err_t check_https_firmware_version() {
  ...
  char const* const version_url = config->version_url;
  ...
  check_and_free((void**)&server_version);
  shared_data_release();
  return err;
}
