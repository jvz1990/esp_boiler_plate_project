...
static void parse_form_data(const char* data, httpd_req_t* req) {
  ...
  if (cJSON_IsArray(networks)) {
    ...
    wifi_settings_t* tmp_ptr = realloc(config->wifi_settings, wifi_count * sizeof(wifi_settings_t));
    if (tmp_ptr == NULL) {
      ESP_LOGE(TAG, "Failed to allocate wifi settings");
      return;
    }
    config->wifi_settings = tmp_ptr;
    config->wifi_configs_count = wifi_count;
    ...
    for (size_t i = 0; i < wifi_count; i++) {
      ...
      if (cJSON_IsString(ssid) && cJSON_IsString(password)) {
        ...
        strlcpy(config->wifi_settings[i].ssid, ssid->valuestring, sizeof(config->wifi_settings[i].ssid));
        strlcpy(config->wifi_settings[i].password, password->valuestring, sizeof(config->wifi_settings[i].password));

        ESP_LOGI(TAG, "SSID: %s, Password: %s", config->wifi_settings[i].ssid, config->wifi_settings[i].password);
      } else {
  ...
  if (cJSON_IsString(ota_url)) {
    ...
    strlcpy(config->ota_url, ota_url->valuestring, sizeof(config->ota_url));
  }
  if (cJSON_IsString(version_url)) {
    ESP_LOGI(TAG, "Version URL: %s", version_url->valuestring);
    strlcpy(config->version_url, version_url->valuestring, sizeof(config->version_url));
  }
